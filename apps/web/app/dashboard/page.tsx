...
function embed(login: string): string {
  const base = appUrl();
  return `<a href="${base}/${login}">\n  <picture>\n    <source media="(prefers-color-scheme: dark)" srcset="${base}/${login}.svg?theme=dark">\n    <img alt="${login}'s GitHub work summary" src="${base}/${login}.svg?theme=light">\n  </picture>\n</a>`;
}
    ...
    <div className="shell dashboard">
      ...
      {!snapshot ? <div className="panel generation-panel"><h2>Building your Ginmap</h2><p className="muted">Ginmap is reading your public GitHub history. A failed backfill resumes from its last completed year.</p>{sync?.error_summary ? <div className="notice">{sync.error_summary}</div> : null}<form action="/api/sync" method="post"><button className="button" type="submit">Queue sync</button></form></div> : (
        <div className="dashboard-grid" style={{ marginTop: 28 }}>
          <aside className="stack">
            <section className="panel">
              ...
              <form action="/api/settings/profile" method="post" className="settings-list">
                <label className="setting-row"><span><strong>Public profile</strong><small>Allow anyone with the URL to view your Ginmap.</small></span><input type="checkbox" name="publicProfile" defaultChecked={settings.publicProfile}/></label>
                <label className="setting-row"><span><strong>Search indexing</strong><small>Allow search engines to index your claimed Ginmap.</small></span><input type="checkbox" name="searchIndexing" defaultChecked={settings.searchIndexing} disabled={!settings.publicProfile}/></label>
          ...
          <div className="stack">
            <section className="panel"><h2>README card</h2><p className="muted">Paste this once. Ginmap keeps the image behind the stable URL up to date.</p>{settings.publicProfile ? <img className="card-preview" src={`/${user.login}.svg?theme=light`} alt="Ginmap card preview" /> : <div className="notice">Enable the public profile to render the card.</div>}<textarea className="embed" readOnly value={embed(user.login)} aria-label="README embed" /></section>
            <section className="panel"><div className="section-heading"><div><h2>Work presentation</h2><p className="muted">Pin representative repositories, reorder pins, or hide noise. These choices affect HTML, SVG, and public JSON without changing GitHub facts.</p></div><form action="/api/sync" method="post"><button className="button" type="submit">Sync now</button></form></div>
