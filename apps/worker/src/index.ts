...
async function reconcileRepositories(userId: string, credential: string): Promise<boolean> {
  ...
  await mapLimit(repositories, 4, async (repository) => {
    ...
    if (!current) { await removeUserRepositoryData(userId, repository.githubId); removed = true; return; }
...
async function refreshContributionHistory(userId: string, credential: string, login: string): Promise<void> {
  ...
  for (const year of contributionYears) await upsertYearContribution(userId, await fetchYearContributions(credential, login, year));
...
async function runReconcile(userId: string, credential: string): Promise<void> {
  ...
  try {
    ...
    for (const year of [current - 1, current].filter((year) => year >= earliest)) await persistYear(userId, credential, user.login, year);
    await refreshOpenPrs(userId, credential, user.login, user.github_created_at);
...
async function serviceCredential(): Promise<string> {
  const token = githubPublicToken();
  if (!serviceTokenVerified) { await verifyPublicCredential(token); serviceTokenVerified = true; }
  return token;
}

async function credentialForUser(userId: string): Promise<{ credential: string; userCredential: boolean }> {
  const credential = await getGitHubCredential(userId);
  if (!credential) return { credential: await serviceCredential(), userCredential: false };
  ...
  if (!needsRefresh) return { credential: decryptToken(credential.tokenCiphertext), userCredential: true };
  ...
  return { credential: refreshed.accessToken, userCredential: true };
}

async function refreshIdentity(userId: string, credential: string, userCredential: boolean): Promise<void> {
  const existing = await getUserById(userId);
  if (!existing) throw new Error("User not found");
  const identity = userCredential ? await fetchViewerIdentity(credential) : await fetchPublicIdentityById(credential, existing.github_id);
...
async function processJob(job: { id: string; userId: string; kind: SyncKind; attempts: number }): Promise<void> {
  const { credential, userCredential } = await credentialForUser(job.userId);
  await refreshIdentity(job.userId, credential, userCredential);
...
for (const signal of ["SIGINT", "SIGTERM"] as const) process.on(signal, () => { stopping = true; healthServer.close(); });
