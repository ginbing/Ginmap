...
function retryDelay(response: Response, attempt: number): number {
  ...
  if (response.headers.get("x-ratelimit-remaining") === "0") {
    ...
    if (resetAt > Date.now()) return resetAt - Date.now() + 1000;
  }
  return Math.max(60_000, 1000 * 2 ** attempt);
}

async function rawGitHubFetch(url: string, credential: string, init: RequestInit = {}, allowedStatuses: number[] = []): Promise<Response> {
...
async function graphql<T>(credential: string, query: string, variables: Record<string, unknown>): Promise<T> {
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    if (!payload.errors?.length && payload.data) return payload.data;
    const rateLimited = response.headers.get("x-ratelimit-remaining") === "0" || payload.errors?.some((error) => /rate limit|secondary rate/i.test(error.message));
    if (rateLimited && attempt < 3) { await sleep(retryDelay(response, attempt)); continue; }
    if (payload.errors?.length) throw new Error(`GitHub GraphQL: ${payload.errors.map((error) => error.message).join("; ")}`);
    throw new Error("GitHub GraphQL returned no data");
  }
  throw new Error("GitHub GraphQL retry budget exhausted");
}
...
function parseOAuthTokenPayload(payload: { access_token?: string; scope?: string; expires_in?: number; refresh_token?: string; refresh_token_expires_in?: number; error?: string; error_description?: string }): OAuthTokenSet {
...
function mapRestUser(user: { id: number; node_id?: string; login: string; avatar_url: string; html_url: string; created_at: string }): GitHubIdentity {
  return { githubId: String(user.id), nodeId: user.node_id ?? null, login: user.login, avatarUrl: user.avatar_url, profileUrl: user.html_url, createdAt: user.created_at };
}

export async function verifyPublicCredential(credential: string): Promise<void> {
  const response = await githubFetch(`${REST}/user`, credential);
  const scopes = response.headers.get("x-oauth-scopes");
  if (scopes == null) throw new Error("GITHUB_PUBLIC_TOKEN must be a dedicated OAuth token with inspectable scopes");
  if (scopes.trim() !== "") throw new Error(`GITHUB_PUBLIC_TOKEN must have no OAuth scopes; received: ${scopes}`);
}

export async function fetchViewerIdentity(credential: string): Promise<GitHubIdentity> {
  const response = await githubFetch(`${REST}/user`, credential);
  return mapRestUser(await response.json() as { id: number; node_id?: string; login: string; avatar_url: string; html_url: string; created_at: string });
}
...
export async function fetchPublicIdentityById(credential: string, githubId: string): Promise<GitHubIdentity | null> {
  const response = await githubFetch(`${REST}/user/${encodeURIComponent(githubId)}`, credential, {}, [404]);
  if (response.status === 404) return null;
  return mapRestUser(await response.json() as { id: number; node_id?: string; login: string; avatar_url: string; html_url: string; created_at: string });
}
...
function mapRepository(repo: GraphRepository): RepositoryIdentity {
  ...
  return {
    githubId: String(repo.databaseId), nodeId: repo.id, ownerLogin: repo.owner.login, ownerId: null, ownerAvatarUrl: repo.owner.avatarUrl,
    name: repo.name, fullName: repo.nameWithOwner, htmlUrl: repo.url, description: repo.description, primaryLanguage: repo.primaryLanguage?.name ?? null,
    stars: repo.stargazerCount, isFork: repo.isFork, isArchived: repo.isArchived,
  };
}

const REPOSITORY_FIELDS = `
  id databaseId name nameWithOwner url description stargazerCount isFork isArchived
  primaryLanguage { name }
  owner { login avatarUrl }
`;

export async function fetchContributionYears(credential: string, login: string): Promise<number[]> {
  const data = await graphql<{ user: { contributionsCollection: { contributionYears: number[] } } | null }>(credential, `query ContributionYears($login: String!) { user(login: $login) { contributionsCollection { contributionYears } } }`, { login });
...
interface ContributionGroup { repository: GraphRepository; contributions: { totalCount: number } }
...
export async function fetchYearContributions(credential: string, login: string, year: number): Promise<YearContributionRecord> {
  ...
  const data = await graphql<{
    user: { contributionsCollection: {
      contributionCalendar: { totalContributions: number }; totalCommitContributions: number; totalIssueContributions: number;
      totalPullRequestContributions: number; totalPullRequestReviewContributions: number; restrictedContributionsCount: number;
      commitContributionsByRepository: ContributionGroup[]; issueContributionsByRepository: ContributionGroup[];
      pullRequestContributionsByRepository: ContributionGroup[]; pullRequestReviewContributionsByRepository: ContributionGroup[];
    }} | null;
  }>(credential, `
  ...
  const apply = (groups: ContributionGroup[], key: "commits" | "issues" | "pullRequests" | "reviews") => {
    for (const group of groups) {
      const repository = mapRepository(group.repository);
      const current = byRepo.get(repository.githubId) ?? { repository, commits: 0, pullRequests: 0, issues: 0, reviews: 0 };
      current[key] += group.contributions.totalCount;
      byRepo.set(repository.githubId, current);
    }
  };
  apply(c.commitContributionsByRepository, "commits");
...
export async function walkAdaptiveWindows<T>(options: { from: Date; to: Date; count: (window: SearchWindow) => Promise<number>; fetch: (window: SearchWindow) => Promise<T[]>; maxResults?: number }): Promise<T[]> {
  ...
  while (queue.length) {
    ...
    if (count === 0) continue;
    if (count <= maxResults) { output.push(...await options.fetch(window)); continue; }
    ...
    queue.unshift({ from: new Date(fromSec * 1000), to: new Date(midSec * 1000) }, { from: new Date((midSec + 1) * 1000), to: new Date(toSec * 1000) });
  }
  return output;
}

function qualifierDate(date: Date): string { return date.toISOString().replace(/\.000Z$/, "Z"); }
...
async function searchCount(credential: string, query: string): Promise<number> {
  const data = await graphql<{ search: { issueCount: number } }>(credential, `query SearchCount($query: String!) { search(query: $query, type: ISSUE, first: 1) { issueCount } }`, { query });
  return data.search.issueCount;
}

interface GraphPr {
  __typename: "PullRequest"; id: string; databaseId: number | null; number: number; title: string; state: "OPEN" | "CLOSED" | "MERGED"; merged: boolean;
...
interface GraphIssue {
  __typename: "Issue"; id: string; databaseId: number | null; number: number; title: string; state: "OPEN" | "CLOSED";
...
async function fetchPrSearchPage(credential: string, query: string, after: string | null): Promise<{ nodes: GraphPr[]; hasNextPage: boolean; endCursor: string | null }> {
  const data = await graphql<{ search: { nodes: Array<GraphPr | null>; pageInfo: { hasNextPage: boolean; endCursor: string | null } } }>(credential, `
    query SearchPrs($query: String!, $after: String) { search(query: $query, type: ISSUE, first: 100, after: $after) {
      pageInfo { hasNextPage endCursor } nodes { ... on PullRequest {
        __typename id databaseId number title state merged additions deletions changedFiles createdAt updatedAt closedAt mergedAt url
...
async function fetchIssueSearchPage(credential: string, query: string, after: string | null): Promise<{ nodes: GraphIssue[]; hasNextPage: boolean; endCursor: string | null }> {
  const data = await graphql<{ search: { nodes: Array<GraphIssue | null>; pageInfo: { hasNextPage: boolean; endCursor: string | null } } }>(credential, `
    query SearchIssues($query: String!, $after: String) { search(query: $query, type: ISSUE, first: 100, after: $after) {
      pageInfo { hasNextPage endCursor } nodes { ... on Issue {
        __typename id databaseId number title state createdAt updatedAt closedAt url repository { ${REPOSITORY_FIELDS} }
      }}
    }}
  `, { query, after });
...
function mapPr(node: GraphPr): PullRequestRecord {
  return {
    nodeId: node.id, databaseId: node.databaseId == null ? null : String(node.databaseId), number: node.number, title: node.title,
    repository: mapRepository(node.repository), state: node.merged ? "MERGED" : node.state, merged: node.merged,
    additions: node.additions, deletions: node.deletions, changedFiles: node.changedFiles, createdAt: node.createdAt, updatedAt: node.updatedAt,
    closedAt: node.closedAt, mergedAt: node.mergedAt, url: node.url,
  };
}
function mapIssue(node: GraphIssue): IssueRecord {
  return {
    nodeId: node.id, databaseId: node.databaseId == null ? null : String(node.databaseId), number: node.number, title: node.title,
    repository: mapRepository(node.repository), state: node.state, createdAt: node.createdAt, updatedAt: node.updatedAt, closedAt: node.closedAt, url: node.url,
  };
}

async function fetchAllPrPages(credential: string, query: string): Promise<PullRequestRecord[]> {
  const items: PullRequestRecord[] = []; let after: string | null = null;
  for (;;) { const page = await fetchPrSearchPage(credential, query, after); items.push(...page.nodes.map(mapPr)); if (!page.hasNextPage || !page.endCursor) break; after = page.endCursor; }
  return items;
}
async function fetchAllIssuePages(credential: string, query: string): Promise<IssueRecord[]> {
  const items: IssueRecord[] = []; let after: string | null = null;
  for (;;) { const page = await fetchIssueSearchPage(credential, query, after); items.push(...page.nodes.map(mapIssue)); if (!page.hasNextPage || !page.endCursor) break; after = page.endCursor; }
  return items;
}
function dedupeByNode<T extends { nodeId: string }>(items: T[]): T[] { return [...new Map(items.map((item) => [item.nodeId, item])).values()]; }
...
export async function fetchPullRequestsInWindow(credential: string, login: string, from: Date, to: Date, field: "created" | "updated" = "created", extra = ""): Promise<PullRequestRecord[]> {
  const items = await walkAdaptiveWindows({ from, to, count: (window) => searchCount(credential, buildSearch(login, "pr", field, window, extra)), fetch: (window) => fetchAllPrPages(credential, buildSearch(login, "pr", field, window, extra)) });
...
export async function fetchIssuesInWindow(credential: string, login: string, from: Date, to: Date, field: "created" | "updated" = "created"): Promise<IssueRecord[]> {
  const items = await walkAdaptiveWindows({ from, to, count: (window) => searchCount(credential, buildSearch(login, "issue", field, window)), fetch: (window) => fetchAllIssuePages(credential, buildSearch(login, "issue", field, window)) });
  return dedupeByNode(items);
}
...
export async function fetchRepositoryMetadata(credential: string, fullName: string): Promise<RepositoryIdentity | null> {
  ...
  const repo = await response.json() as {
    id: number; node_id?: string; name: string; full_name: string; html_url: string; description: string | null; language: string | null;
    stargazers_count: number; fork: boolean; archived: boolean; owner: { login: string; id: number; avatar_url: string };
  };
  return {
    githubId: String(repo.id), nodeId: repo.node_id ?? null, ownerLogin: repo.owner.login, ownerId: String(repo.owner.id), ownerAvatarUrl: repo.owner.avatar_url,
    name: repo.name, fullName: repo.full_name, htmlUrl: repo.html_url, description: repo.description, primaryLanguage: repo.language,
    stars: repo.stargazers_count, isFork: repo.fork, isArchived: repo.archived,
  };
}
